...
public class AggregateOperators
{
    ...
    public int NestedCount()
    {
        ...
        var orderCounts = customers.Select(c =>
    ...
    public int GroupedCount()
    {
        ...
        var categoryCounts = products.GroupBy(p => p.Category).Select(g =>
            Tuple.Create(g.Key, g.Count())).ToList();
    ...
    {
        List<Product> products = GetProductList();
        var categories = products.GroupBy(p => p.Category).Select(g =>
            Tuple.Create(g.Key, g.Sum(p => p.UnitsInStock))).ToList();
    ...
    public int MinGrouped()
    {
        ...
        var categories = products.GroupBy(p => p.Category).Select(g =>
            Tuple.Create(g.Key, g.Min(p => p.UnitPrice))).ToList();
    ...
    public int MinEachGroup()
    {
        List<Product> products = GetProductList();
        var categories = products.GroupBy(p => p.Category).Select(g =>
            Tuple.Create(g.Key, g.Where(p => p.UnitPrice == g.Min(p => p.UnitPrice)).ToList())).ToList();
    ...
    public int MaxGrouped()
    {
        ...
        var categories = products.GroupBy(p => p.Category).Select(g =>
            Tuple.Create(g.Key, g.Max(p => p.UnitPrice))).ToList();
    ...
    public int MaxEachGroup()
    {
        ...
        var categories = products.GroupBy(p => p.Category).Select(g =>
            Tuple.Create(g.Key, g.Where(p => p.UnitPrice == g.Max(p => p.UnitPrice)).ToList())).ToList();
    ...
    public int AverageGrouped()
    {
        ...
        var categories = products.GroupBy(p => p.Category).Select(g =>
            Tuple.Create(g.Key, g.Average(p => p.UnitPrice))).ToList();
